...
if n_type == "vit":
    n_type = "vit"
    vit_jacc = normalize(np.load(dir_+"jaccard_"+task+"_"+n_type+".npy"))
    n_labels = vit_jacc.shape[-1]
else:
    n_type = "mlp"
...
for k, sp in enumerate(sparsity):
    ...
    for i in range(4):
        if n_type != "vit":
            ...
            mu_vit = vit_jacc[k, i].mean(axis=0)
            ax.bar(x[ii],
                   mu_vit,
                   width=0.8,
                   color=ccolor[ii])
            ax.set_xticks([])
            ax.set_title("Sparsity $p = $"+str(sparsity[k]), fontsize=14)
        ii += 1
